...
docker exec -i -u postgres "$container" /ros_entrypoint.sh python3 - < "$(dirname "$0")/publish-smoke.py"
...
echo 'Smoke passed: automatic startup, graph additions/removals, topic notifications, publish delivery, atomic rollback, worker recovery, reader access, reinstall, and server restart'
